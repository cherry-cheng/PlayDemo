...
public class SmartViewPager extends FrameLayout {
    ...
    private void initView() {
        ...
        textLp.gravity = Gravity.BOTTOM | Gravity.LEFT;
        int textbottomMargin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 8, getResources().getDisplayMetrics());
        textLp.bottomMargin = textbottomMargin;
        textLp.leftMargin = textbottomMargin;
...
//            mIndicator.setPositionAndOffset(position, positionOffset);
            ...
            if (App.getData() != null) {
                if (position >= App.getData().size()) {
                    title.setText(App.getData().get(App.getData().size() - 1).getTitle());
                } else {
                    title.setText(App.getData().get(position).getTitle());
                }
            }
            Log.e("cyh666", "position = " + position + "positionOffset = " + positionOffset + "positionOffsetPixels = " + positionOffsetPixels);
        }

        @Override
        public void onPageSelected(int position) {
            Log.e("cyh666", "position = " + position);
